...
def draw_board(state, std, player_id_str, current_direction):
    ...
    for row_offset in range(Constants.VIEW_HEIGHT):
        ...
        for col_offset in range(Constants.VIEW_WIDTH):
            col = left + col_offset
            cell = (row, col)
            cell_list = [row, col]  # Также проверяем как список
            ...
            for i, part in enumerate(my_snake):
                if (cell == tuple(part) if isinstance(part,
                                                      list) else cell == part):
                    is_my_snake = True
            ...
            # Проверка, является ли клетка едой
            if isinstance(food, list) and len(food) >= 2 and all(
                    isinstance(x, (int, float)) for x in food[:2]):
                # Еда представлена как [row, col]
                food_cell = tuple(food[:2])
                if cell == food_cell:
                    symbol = Constants.FOOD_CHAR1
                    color = curses.color_pair(2)
            else:
                # Другие форматы еды
                for food_item in food if isinstance(food, list) else []:
                    if isinstance(food_item, list) and len(food_item) >= 2:
                        if cell == tuple(food_item[:2]):
                            symbol = Constants.FOOD_CHAR1
                            color = curses.color_pair(2)
                            break
                    elif isinstance(food_item,
                                    dict) and 'position' in food_item:
                        pos = food_item['position']
                        if cell == tuple(pos) if isinstance(pos,
                                                            list) else cell == pos:
                            symbol = Constants.FOOD_CHAR1
                            color = curses.color_pair(2)
                            break
            ...
            if cell in obstacles or tuple(cell_list) in obstacles:
    ...
    std.addstr(12, Constants.VIEW_WIDTH + 5, f"Игрок ID: {player_id_str}")
    ...
    std.addstr(18, Constants.VIEW_WIDTH + 5, f"Всего змей: {len(snakes)}")
